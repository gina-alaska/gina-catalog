class Manager::EntriesController < ApplicationController
  ...
  def entry_params
    params.require(:entry).permit(:title, :description, :status, :entry_type_id, :start_date, :end_date, :use_agreement_id, :request_contact_info, :require_contact_info, :tag_list, attachments_attributes: [:id, :file, :description, :category, :_destroy], entry_contacts_attributes: [:id, :contact_id, :primary, :secondary, :_destroy], entry_agencies_attributes: [:id, :agency_id, :primary, :funding, :_destroy])
  end
end
