module GitHttp
  class App 
    ...
    def get_git_dir(path)
      ...
      path = File.join(root, path) unless path[0] == ?/
    ...
    def match_routing
      ...
      SERVICES.each do |method, handler, match, rpc|
        match = "#{@config[:uri_root]}/#{match}" if @config[:uri_root]
        if m = Regexp.new(match).match(@req.path_info)
          return ['not_allowed'] if method != @req.request_method
          cmd = handler
          path = m[1]
          
          slug = File.basename(path, '.git')
          repo = Repo.where(slug: slug).first

          file = @req.path_info.sub(path + '/', '')
          file = file.sub(@config[:uri_root] + '/', '') if @config[:uri_root]
          
          return [cmd, repo.path, file, rpc]
        end
      end
      return nil
    end
