module CmsConcerns
  ...
  def export_pages(pages)
    ...
    pages.includes(:cms_layout).each do |page|
      page_list << page.as_json(only: %i[title content hidden redirect_url slug], methods: %i[parent_ancestry_path cms_layout_name])
  ...
  def export_cms
    json = {
      layouts: layouts.as_json(only: %i[name content]),
      snippets: snippets.as_json(only: %i[name content]),
      themes: themes.as_json(only: %i[name css]),
