class Contact < ActiveRecord::Base
  ...
  scope :used_by_portal, ->(portal) {
    includes(:entry_portals).references(:entry_portals).where("portal_id = :portal_id or contacts.created_at >= :start_date", portal_id: portal.id, start_date: 1.minute.ago)
  }
