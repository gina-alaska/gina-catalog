...
execute 'bundle_install' do
  command "chruby-exec #{node['glynx']['ruby_version']} -- bundle install --path=./.gems --verbose > /tmp/gemlog"
...
execute 'glynx_rake_db_setup' do
  command "chruby-exec #{node['glynx']['ruby_version']} -- bundle exec rake db:setup"
  cwd node['glynx']['deploy_path']
  user node['glynx']['account']
  group node['glynx']['account']
  only_if { node['glynx']['database']['setup'] }
end
...
execute 'solr_reindex' do
  ...
  action :nothing
  not_if { ::File.exist?("/home/#{node['glynx']['account']}/solr_noreindex") }
end
